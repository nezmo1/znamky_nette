...
function getMonthYearString(dt) {
  return ['Leden','Únor','Březen','Duben','Květen','Červen','Červenec',
          'Srpen','Září','Říjen','Listopad','Prosinec'][dt.getMonth()] +
         ' ' + dt.getFullYear();
}
...
function createCalendar(div, month) {
  ...
  daysRow.insertCell(-1).innerHTML="Po";
  daysRow.insertCell(-1).innerHTML="Út";
  daysRow.insertCell(-1).innerHTML="St";
  daysRow.insertCell(-1).innerHTML="Čt";
  daysRow.insertCell(-1).innerHTML="Pá";
  daysRow.insertCell(-1).innerHTML="So";
  daysRow.insertCell(-1).innerHTML="Ne";
  ...
  for (i=0; i<allElements.length; i++) {
    ...
    if (className=='datepicker' || className.indexOf('datepicker ') != -1 || className.indexOf(' datepicker') != -1) {
      ...
      a.setAttribute('onclick','return showDatePicker("' + allElements[i].id + '")');
       a.setAttribute('class','col-md-6 col-sm-6 col-xs-12');
       a.setAttribute('style','z-index:1;');
