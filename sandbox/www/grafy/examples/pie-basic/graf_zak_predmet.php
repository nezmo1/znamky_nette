...
$(function () {
        $('#container').highcharts({
            ...
            series: [{
                ...
                data: [
                <?php
                $sql_znamky_z = "SELECT z.zak ,count(znamka) as `znamka_s`, p.nazev as `nazev` FROM `zak_znam` as `z` 
join predmety as `p` on predmety_predmet_id=p.predmet_id
WHERE zak='".$_SESSION['id']."'
GROUP BY zak, nazev"; 

$sql_pocet_znamek="SELECT count(zak) as `pocet_znamek` FROM zak_znam WHERE zak='".$_SESSION['id']."'";
