<?php
// source: D:\xampp\htdocs\znamky_nette\sandbox\vendor\o5\grido\Grido/Grid.latte

// prolog Latte\Macros\CoreMacros
list($_b, $_g, $_l) = $template->initialize('4640391683', 'html')
;
// prolog Latte\Macros\BlockMacros
//
// block _grid
//
if (!function_exists($_b->blocks['_grid'][] = '_lb34a880e34c__grid')) { function _lb34a880e34c__grid($_b, $_args) { foreach ($_args as $__k => $__v) $$__k = $__v; $_control->redrawControl('grid', FALSE)
...
<?php $iterations++; } } Nette\Bridges\FormsLatte\FormMacros::renderFormBegin($form = $_form = $_control["form"], array()) ;if ($filterRenderType == \Grido\Components\Filters\Filter::RENDER_OUTER) { call_user_func(reset($_b->blocks['outerFilter']), $_b, get_defined_vars()) ; } ?>
...
// block outerFilter
//
if (!function_exists($_b->blocks['outerFilter'][] = '_lb2d571c3e5b_outerFilter')) { function _lb2d571c3e5b_outerFilter($_b, $_args) { foreach ($_args as $__k => $__v) $$__k = $__v
...
// block table
//
if (!function_exists($_b->blocks['table'][] = '_lb06a0a492ca_table')) { function _lb06a0a492ca_table($_b, $_args) { foreach ($_args as $__k => $__v) $$__k = $__v
...
1<?php } else { ?>2<?php } ?>"<?php } ?>>
                <input type="checkbox" title="<?php echo Latte\Runtime\Filters::escapeHtml($template->translate('Invert'), ENT_COMPAT) ?>">
            </th>
...
<?php $iterations++; } if ($showActionsColumn) { ?>            <th class="actions center">
                <?php echo Latte\Runtime\Filters::escapeHtml($template->translate('Actions'), ENT_NOQUOTES) ?>
...
<?php if (is_string($column->getCustomRender()) && $column->getCustomRenderVariables()) { $_b->templates['4640391683']->renderChildTemplate($column->getCustomRender(), array_merge(array('control' => $control, 'presenter' => $control->getPresenter(), 'item' => $row, 'column' => $column, ), $column->getCustomRenderVariables(), array()) + $template->getParameters()) ;} elseif (is_string($column->getCustomRender())) { $_b->templates['4640391683']->renderChildTemplate($column->getCustomRender(), array('control' => $control, 'presenter' => $control->getPresenter(), 'item' => $row, 'column' => $column) + $template->getParameters()) ;} else { ?>
...
" class="no-results"><?php echo Latte\Runtime\Filters::escapeHtml($template->translate('No results.'), ENT_NOQUOTES) ?></td></tr>
<?php } ?>
    </tbody>
...
// block action
//
if (!function_exists($_b->blocks['action'][] = '_lbe046891e74_action')) { function _lbe046891e74_action($_b, $_args) { foreach ($_args as $__k => $__v) $$__k = $__v
...
// block operations
//
if (!function_exists($_b->blocks['operations'][] = '_lb64cfb7d548_operations')) { function _lb64cfb7d548_operations($_b, $_args) { foreach ($_args as $__k => $__v) $$__k = $__v
;if ($operation) { ?>                <span class="operations"  title="<?php echo Latte\Runtime\Filters::escapeHtml($template->translate('Select some row'), ENT_COMPAT) ?>">
...
// block paginator
//
if (!function_exists($_b->blocks['paginator'][] = '_lb4f6f422006_paginator')) { function _lb4f6f422006_paginator($_b, $_args) { foreach ($_args as $__k => $__v) $$__k = $__v
...
"><i class="glyphicon glyphicon-arrow-left fa fa-arrow-left icon-arrow-left"></i> <?php echo Latte\Runtime\Filters::escapeHtml($template->translate('Previous'), ENT_NOQUOTES) ?></span>
<?php } else { ?>
...
"><i class="glyphicon glyphicon-arrow-left fa fa-arrow-left icon-arrow-left"></i> <?php echo Latte\Runtime\Filters::escapeHtml($template->translate('Previous'), ENT_NOQUOTES) ?></a>
...
<?php } if ($iterator->nextValue > $step + 1) { ?>                        <a class="prompt" data-grido-prompt="<?php echo Latte\Runtime\Filters::escapeHtml($template->translate('Enter page:'), ENT_COMPAT) ?>
...
<?php } $prevStep = $step ;$iterations++; } array_pop($_l->its); $iterator = end($_l->its) ;if ($control->page == $paginator->getPageCount()) { ?>
                        <span class="btn btn-default btn-xs btn-mini disabled" href="<?php echo Latte\Runtime\Filters::escapeHtml($_control->link("page!", array('page' => $paginator->getPage() + 1)), ENT_COMPAT) ?>
"><?php echo Latte\Runtime\Filters::escapeHtml($template->translate('Next'), ENT_NOQUOTES) ?> <i class="glyphicon glyphicon-arrow-right fa fa-arrow-right icon-arrow-right"></i></span>
<?php } else { ?>
                        <a class="btn btn-default btn-xs btn-mini ajax" href="<?php echo Latte\Runtime\Filters::escapeHtml($_control->link("page!", array('page' => $paginator->getPage() + 1)), ENT_COMPAT) ?>
"><?php echo Latte\Runtime\Filters::escapeHtml($template->translate('Next'), ENT_NOQUOTES) ?> <i class="glyphicon glyphicon-arrow-right fa fa-arrow-right icon-arrow-right"></i></a>
<?php } ?>
                </span>
<?php } 
}}
...
// block count
//
if (!function_exists($_b->blocks['count'][] = '_lbd8a09d2558_count')) { function _lbd8a09d2558_count($_b, $_args) { foreach ($_args as $__k => $__v) $$__k = $__v
?>                <span class="count">
                    <?php echo Latte\Runtime\Filters::escapeHtml(sprintf($template->translate('Items %d - %d of %d'), $paginator->getCountBegin(), $paginator->getCountEnd(), $control->getCount()), ENT_NOQUOTES) ?>
...
" title="<?php echo Latte\Runtime\Filters::escapeHtml($template->translate('Export all items'), ENT_COMPAT) ?>"><i class="glyphicon glyphicon-download fa fa-download icon-download"></i></a>
<?php } ?>
                </span>
<?php
}}
