...
class SeznamCvZnamekUcitel extends Grid{
    ...
    protected function configure($presenter)
    {
          ...
          $predmety=  $this->database->query('SELECT DISTINCT predmet, p.nazev AS `nazev` FROM prum_znamky INNER JOIN predmet as `p` on predmet=p.id_predmetu where ucitel= ?',$this->ucitel->id,' ORDER BY nazev');
          $zaci=  $this->database->query('SELECT DISTINCT zak,u.jmeno AS `jmeno`, u.prijmeni AS `prijmeni` FROM prum_znamky INNER JOIN users AS `u` ON zak=u.id_users WHERE ucitel= ?',$this->ucitel->id,' ORDER BY prijmeni');      
