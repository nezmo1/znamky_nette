...
class SeznamZnamekUcitelSestava extends Grid{
    ...
    protected function configure($presenter)
    {
          ...
          $predmety=  $this->database->query('SELECT DISTINCT predmet, p.nazev AS `nazev` FROM znamky INNER JOIN predmet as `p` on predmet=p.id_predmetu ORDER BY nazev');
          $zaci=  $this->database->query('SELECT DISTINCT zak,u.jmeno AS `jmeno`, u.prijmeni AS `prijmeni` FROM znamky INNER JOIN users AS `u` ON zak=u.id_users  ORDER BY prijmeni');      
          $data= $this->database->query('SELECT DISTINCT datum, DATE_FORMAT(datum,"%d-%m-%Y") as `data` FROM znamky ORDER BY datum');  
